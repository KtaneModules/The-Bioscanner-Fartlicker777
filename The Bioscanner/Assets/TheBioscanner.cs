...
#pragma warning restore 414
   ...
   void Start () {
      ...
      if (Bomb.IsPortPresent(Port.StereoRCA)) { GlyphNumbers[2] += Bomb.GetPortCount(Port.StereoRCA); }
      else if (Bomb.IsPortPresent(Port.Serial)) { GlyphNumbers[2] += Bomb.GetPortCount(Port.Serial); }
      else if (Bomb.IsPortPresent(Port.RJ45)) { GlyphNumbers[2] += Bomb.GetPortCount(Port.RJ45); }
      else if (Bomb.IsPortPresent(Port.PS2)) { GlyphNumbers[2] += Bomb.GetPortCount(Port.PS2); }
      else if (Bomb.IsPortPresent(Port.Parallel)) { GlyphNumbers[2] += Bomb.GetPortCount(Port.Parallel); }
      else if (Bomb.IsPortPresent(Port.DVI)) { GlyphNumbers[2] += Bomb.GetPortCount(Port.DVI); }
      ...
      Debug.LogFormat("[The Bioscanner #{0}] The first glyph is {1}{2}.", moduleId, Alphabet[GlyphNumbers[0] % 5], (GlyphNumbers[0] / 5) + 1);
      Debug.LogFormat("[The Bioscanner #{0}] The second glyph is {1}{2}.", moduleId, Alphabet[GlyphNumbers[1] % 5], (GlyphNumbers[1] / 5) + 1);
      Debug.LogFormat("[The Bioscanner #{0}] The third glyph is {1}{2}.", moduleId, Alphabet[GlyphNumbers[2] % 5], (GlyphNumbers[2] / 5) + 1);
   }
   ...
   void Update () {
      if (Started) {
         ...
         if (TheTimerForThis < 0) {
            ...
            for (int i = 0; i < 10; i++) {
               Pressed[i] = false;
               ButtonOutlinesButColors[i].material.color = new Color32(255, 0, 0, 255);
            }
            Started = false;
         }
      }
...
#pragma warning restore 414
   ...
   IEnumerator ProcessTwitchCommand (string Command) {
      ...
      yield return null;
      if (Command == "START" || Command == "ACTIVATE") { StartButton.OnInteract(); }
      else if (Command == "A1") { Buttons[4].OnInteract(); }
      else if (Command == "A2") { Buttons[5].OnInteract(); }
      else if (Command == "A3") { Buttons[6].OnInteract(); }
      else if (Command == "B1") { Buttons[0].OnInteract(); }
      else if (Command == "B2") { Buttons[1].OnInteract(); }
      else if (Command == "B3") { Buttons[2].OnInteract(); }
      else if (Command == "B4") { Buttons[3].OnInteract(); }
      else if (Command == "C1") { Buttons[7].OnInteract(); }
      else if (Command == "C2") { Buttons[8].OnInteract(); }
      else if (Command == "C3") { Buttons[9].OnInteract(); }
      else { yield return "sendtochaterror I don't understand!"; }
   }
